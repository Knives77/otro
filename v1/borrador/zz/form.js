...
for (let index = 1; index <= 30; index++) {
    document.getElementById("sele").innerHTML += `<option id="${index}">${index} </option>`;
}
...
function funhh() {
    ...
    else if(!c.test(apellidos))
    {
       ...
       x = false;
        ...
        alert("La contaseña debe de contener: letras mayusculas, minusculas, números y caracteres especiales ");
        x = false;
    }
    return x;
}
